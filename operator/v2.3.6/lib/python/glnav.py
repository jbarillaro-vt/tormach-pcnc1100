...
class GlNavBase:
    ...
    def zoomin(self, ratio=1.1):
        self.distance = self.distance / ratio
        self._redraw()

    def zoomout(self, ratio=1.1):
        self.distance = self.distance * ratio
        self._redraw()
    ...
    def set_view_y(self):
        ...
        if self.is_lathe():
            glRotatef(90, 0, 1, 0)
        mid, size = self.extents_info()
        glTranslatef(x_dir * -mid[0], -mid[1], -mid[2])
    ...
    def set_view_p(self):
        ...
        if self.canon:
            ...
            size = ((size[0] ** 2 + size[1] ** 2 + size[2] ** 2) ** .5) / .55
